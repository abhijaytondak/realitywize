...
export default function AdminDashboard() {
  const activeProperties = SAMPLE_PROPERTIES.filter((p) => p.is_active).length;
  const featuredProperties = SAMPLE_PROPERTIES.filter((p) => p.is_featured).length;

  const stats = [
    { label: "Total Properties", value: SAMPLE_PROPERTIES.length, href: "/admin/properties", color: "bg-primary-fixed/30 text-primary" },
    ...
    { label: "Enquiries", value: 0, href: "/admin/enquiries", color: "bg-blue-50 text-blue-700" },
  ];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Link
          ...
          className="bg-white rounded-xl p-6 border border-outline-variant/20 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4"
        >
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-on-surface-variant">Check new leads and messages</p>
          </div>
        </Link>
        <Link
          href="/admin/settings"
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-on-surface-variant">Update contact info and config</p>
          </div>
        </Link>
      </div>
    </div>
